...
const usedHullNumbers = new Set(
  ['YW-10001', 'AZ60-2025-01', 'BEN-OASIS-7', 'SUN-55-001'].map(normalize)
)
...
function renderWarrantyToggles() {
  warrantyToggles.forEach((toggle) => {
    ...
    toggle.setAttribute(
      'aria-label',
      `${card.querySelector('p').textContent.trim()} ${enabled ? 'enabled' : 'disabled'}`
    )
...
function getModelGroups(filter = '') {
  ...
  const mine = sortAlpha(state.userModels[state.make] || []).filter(
    (model) => !officialNames.has(normalize(model))
  )

  return [
    {
      title: 'My Created Models',
      options: mine.filter(matches(filter)).map((value) => ({ value, source: 'mine' }))
    },
...
function validateGeneralInfo() {
  ...
  if (!hull) {
    ...
    setError(
      hullField,
      'Hull or production number already exists. Enter a unique number to continue.'
    )
  }
...
function clearAllErrors() {
  ;[
    ...
    combos.year.field,
    hullField,
    document.querySelector('.modal-field')
  ].forEach(clearError)
}
...
function readUserModels() {
  ...
  return Object.fromEntries(
    Object.entries(saved).map(([make, models]) => [
      make,
      Array.isArray(models) ? sortAlpha(models) : []
    ])
  )
}

function buildSlug() {
  return [state.year, state.make, state.model, hullInput.value.trim()]
    .map(slugify)
    .filter(Boolean)
    .join('-')
}
